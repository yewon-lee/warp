...
class Example:
    ...
    def load_mesh(self, filename):
        if use_meshio:
            ...
            mesh_indices = np.array(
                m.face_vertex_indices(), dtype=np.int32).flatten()
    ...
    def __init__(self, stage):
        ...
        # self.load_volume(os.path.join(os.path.dirname(__file__), f"assets/bowl.nvdb"))
        bowl_volume = None
        bowl_mesh = self.load_mesh(os.path.join(
            os.path.dirname(__file__), f"assets/bowl.obj"))

        # self.load_volume(os.path.join(os.path.dirname(__file__), f"assets/{model_name}.nvdb"))
        body1_volume = None
        body1_mesh = self.load_mesh(os.path.join(
            os.path.dirname(__file__), f"assets/{model_name}.obj"))

        # self.load_volume(os.path.join(os.path.dirname(__file__), f"assets/{model_name}.nvdb"))
        body2_volume = None
        body2_mesh = self.load_mesh(os.path.join(
            os.path.dirname(__file__), f"assets/{model_name}.obj"))

        # self.load_volume(os.path.join(os.path.dirname(__file__), f"assets/{model_name}.nvdb"))
        body3_volume = None
        body3_mesh = self.load_mesh(os.path.join(
            os.path.dirname(__file__), f"assets/{model_name}.obj"))
        ...
        bowl = builder.add_body(
            origin=wp.transform(
                (0.0, 1.1, 0.0), wp.quat_from_axis_angle((1.0, 0.0, 0.0), 0.01))
        )
        builder.add_shape_mesh(
            body=bowl,
            mesh=bowl_mesh,
        ...
        #     iterations=1, contact_con_weighting=False)
        ...
        with wp.ScopedTimer("simulate", active=False):
            ...
            for i in range(self.sim_substeps):
                ...
                wp.sim.collide(self.model, self.state,
                               experimental_sdf_collision=True)

                if not self.use_cuda_graphs and i == 0:
                    # render contact points
                    qs = self.state.body_q.numpy()
                    rigid_contact_count = self.model.rigid_contact_count.numpy()[
                        0]
                    self.max_contact_count = max(
                        self.max_contact_count, rigid_contact_count)
                    ...
                    if rigid_contact_count > 0:
                        ...
                        self.points_a[:rigid_contact_count] = [wp.transform_point(
                            qs[body], wp.vec3(*contact_points_a[i])) for i, body in enumerate(body_a)]

                        contact_points_b = self.model.rigid_contact_point1.numpy()
                        self.points_b[:rigid_contact_count] = [wp.transform_point(
                            qs[body], wp.vec3(*contact_points_b[i])) for i, body in enumerate(body_b)]

                    self.render()

                self.state = self.integrator.simulate(
                    self.model, self.state, self.state, self.sim_dt/self.sim_substeps)
    ...
    def render(self, is_live=False):
        ...
        with wp.ScopedTimer("render", active=False):
            ...
            self.renderer.render_points(
                "contact_points_a", np.array(self.points_a), radius=0.05)
            self.renderer.render_points(
                "contact_points_b", np.array(self.points_b), radius=0.05)
...
if __name__ == '__main__':
    stage_path = os.path.join(os.path.dirname(
        __file__), "outputs/example_rigid_sdf_contact.usd")
    ...
    num_con_history.append(
        example.model.rigid_contact_inv_weight.numpy().copy())
    ...
    if True:
        ...
        fig, ax = plt.subplots(example.model.body_count,
                               7, figsize=(10, 10), squeeze=False)
